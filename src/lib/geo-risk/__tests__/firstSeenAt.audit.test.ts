 ...
 * D-GRF11(2026-07-20): H3(커서 유실 버그)는 이 세션에서 수정됨(router.ts/d1.ts/telegramFetch.ts).
 * 아래 두 번째 describe는 "버그가 있었다"가 아니라 "버그가 고쳐졌다"를 증명한다.
 */
...
describe("H2(미수정, 저우선): firstSeenAt은 실제 최초 관측시각(rep.receivedAt)이 아니라 처리시각(nowIso)", () => {
  it("가장 이른 메시지보다 firstSeenAt이 늦게 찍힌다(처리 지연만큼 부정확) — sourceRef에 진짜 값은 복구 가능", () => {
    ...
    // 버킷 대표(rep)는 가장 이른 알림 — sourceRef로 실제 관측시각 복구 가능(id="channel/receivedAt").
    expect(events[0].sourceRef).toBe("A/1");
    // firstSeenAt은 rep.receivedAt(실제 첫 관측)이 아니라 cronProcessedAt(처리시각).
    ...
    expect(lagMs).toBe(10 * 60 * 1000 + 5000); // ~cron 주기만큼 조직적 지연(D-GRF2 n=618일 대비 무시 가능)
  });
});
