...
const elevatedCache = new Map<string, TransportPathPoint[]>();
const MAX_CACHE = 128;
...
export function buildElevatedNeptunPath(
  ...
  const cached = elevatedCache.get(key);
  ...
  elevatedCache.set(key, points);
  if (elevatedCache.size > MAX_CACHE) {
    const oldest = elevatedCache.keys().next().value;
    if (oldest) elevatedCache.delete(oldest);
  }
