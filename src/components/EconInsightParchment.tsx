...
function riskLabel(level: EconInsightBrief["riskLevel"], lang: LabelLanguage): string {
  ...
  if (level === "CRITICAL") return "?ÑÌóò??¬∑ ÏπòÎ™Ö";
  if (level === "HIGH") return "?ÑÌóò??¬∑ ?íÏùå";
  return "?ÑÌóò??¬∑ ?àÏ†ï";
}
              ...
              (gdp ||
                ...
                <div className="mt-4 rounded-sm border border-[#8b6914]/28 bg-[#f6ebcf]/55 px-3 py-2.5 text-center text-[0.82rem] leading-relaxed text-[#4a3724]">
                  ...
                  >
                    ...
                    {gdpPc ? (
                      <span>
                        {lang === "en" ? "GDP/cap" : "1?∏Îãπ"} {gdpPc}
                      </span>
                    ) : null}
                    {growth ? (
                      <span>
                        {lang === "en" ? "Growth" : "?±Ïû•"} {growth}
                      </span>
                    ) : null}
                    {inflation ? (
                      <span>
                        {lang === "en" ? "CPI" : "?∏Ìîå??} {inflation}
                      </span>
                    ) : null}
                    {unemp ? (
                      <span>
                        {lang === "en" ? "Unemp" : "?§ÏóÖ"} {unemp}
                      </span>
                    ) : null}
                    {worldStats.population != null ? (
                      <span>
                      ...
                      {inflShock?.rangePp != null ? (
                        <p>
                          {lang === "en"
                            ...
                            : `?∏Ìîå??Ï∞? ${inflShock.min?.toFixed(1)}??{inflShock.max?.toFixed(1)}% (Î≤îÏúÑ ${inflShock.rangePp.toFixed(1)}%p)${
                                inflShock.deltaPp != null
                                  ? ` ¬∑ ?ÑÎÖÑÎπ?${formatSignedPct(inflShock.deltaPp)}p`
                                  : ""
                              }`}
                        </p>
                      ) : null}
                      {growthShock?.rangePp != null ? (
                        <p>
                          {lang === "en"
                            ...
                            : `?±Ïû• Ï∞? ${growthShock.min?.toFixed(1)}??{growthShock.max?.toFixed(1)}% (Î≤îÏúÑ ${growthShock.rangePp.toFixed(1)}%p)${
                                growthShock.deltaPp != null
                                  ? ` ¬∑ ?ÑÎÖÑÎπ?${formatSignedPct(growthShock.deltaPp)}p`
                                  : ""
                              }`}
                        </p>
                      ) : null}
                ...
                {typingDone && macroNarrative.length > 0 ? (
                  <div className="space-y-3 border-t border-[#8b6914]/20 pt-3">
                    <p
                      ...
                      style={{ fontFamily: PARCHMENT_STACK, fontWeight: 500 }}
                    >
                      {lang === "en" ? "Country macro ¬∑ live" : "Íµ?? Í±∞Ïãú ¬∑ ?ºÏù¥Î∏?}
                    </p>
