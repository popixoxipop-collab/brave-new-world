// 육상 국경선 JSON만 생성: node scripts/build-country-borders.js
const fs = require("fs");
...
const BORDER_SHP = path.join(CULTURAL_DIR, "ne_10m_admin_0_boundary_lines_land.shp");
const OUT_FILE = path.join(OUT_DIR, "country-borders.json");

const BORDER_LINE_MAX_POINTS = IS_LITE ? PROFILE.countryBorderLineMaxPoints : 48;
const BORDER_COORD_PRECISION = 2;
...
function lineGeometryToPaths(geometry) {
  ...
  if (geometry.type === "LineString") {
    return [
      toPoints(simplifyLine(geometry.coordinates, BORDER_LINE_MAX_POINTS, BORDER_COORD_PRECISION)),
    ];
  }
  if (geometry.type === "MultiLineString") {
    return geometry.coordinates.map((line) =>
      toPoints(simplifyLine(line, BORDER_LINE_MAX_POINTS, BORDER_COORD_PRECISION)),
    );
  }
  return [];
}
...
async function fromShapefile() {
  const source = await shapefile.open(BORDER_SHP);
  const paths = [];
  let index = 0;
...
async function main() {
  // MVP: 10m shapefile 대신 Natural Earth 110m GeoJSON 사용 (정점 수 대폭 감소)
  const paths = await fromGeoJsonUrl();

  fs.mkdirSync(path.dirname(OUT_FILE), { recursive: true });
  writeJsonArrayFile(
    OUT_FILE,
    paths.map((path) => compactTransportPath(path, { precision: BORDER_COORD_PRECISION })),
  );
  console.log(`육상 국경선(110m) ${paths.length}개 → ${OUT_FILE}`);
}
